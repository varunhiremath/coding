...
public:
  ...
  friend std::ostream &operator<<(std::ostream &out, BinarySearchTree<T> const &bst)
  {
    size_t const height = bst.height(bst._head);
    std::vector<Node<T>*> nodes{bst._head};
    bst.prettyPrintTree(nodes, 0, height, 2);
    return out;
  }
